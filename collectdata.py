...
while True:
    ...
    count = {
             ...
             'A': len(os.listdir(directory+"/Abhit")),
             'H': len(os.listdir(directory+"/How")),
             'Y': len(os.listdir(directory+"/Are You"))
             }
    ...
    if interrupt & 0xFF == ord('A'):
        cv2.imwrite(directory+'Abhit/'+str(count['abhit'])+'.png',frame)
    if interrupt & 0xFF == ord('H'):
        cv2.imwrite(directory+'How/'+str(count['how'])+'.png',frame)
    if interrupt & 0xFF == ord('Y'):
        cv2.imwrite(directory+'Are You/'+str(count['are you'])+'.png',frame)
